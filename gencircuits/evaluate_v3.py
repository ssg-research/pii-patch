...
model = AutoModelForCausalLM.from_pretrained("/mnt/d1/acp23ajh/" + model_name)
...
os.makedirs(f"graphs/{model_name_noslash}", exist_ok=True)
graph.to_json(f"graphs/{model_name_noslash}/{task}_kl.json")
...
later_steps = list(range(s, e, 100))
...
os.makedirs(f"results/pareto/{model_name_noslash}", exist_ok=True)
