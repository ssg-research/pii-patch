...
def get_logit_positions(logits: torch.Tensor, input_length: torch.Tensor):
    ...
    # Ensure input_length is valid
    if (input_length <= 0).any():
        raise ValueError(f"input_length must be > 0, got: {input_length}")
    if (input_length > seq_len).any():
        # add padding to the seq_len
        # padding = torch.zeros(batch_size, seq_len - input_length.max(), device=logits.device)
        # logits = torch.cat([logits, padding], dim=1)
        
        raise ValueError(
            f"input_length {input_length.max()} exceeds sequence length {seq_len}"
        )
...
def divergence(
    ...
    if divergence_type == "kl":
        ...
        log_clean_probs = torch.log_softmax(clean_logits, dim=-1)
        results = kl_div(
            log_probs, log_clean_probs, log_target=True, reduction="none"
        ).sum(-1)
...
def get_singular_and_plural(model, strict=False) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    df: pd.DataFrame = pd.read_csv("./gencircuits/data/sva/combined_verb_list.csv")
