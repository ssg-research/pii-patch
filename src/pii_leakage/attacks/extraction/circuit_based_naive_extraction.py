...
from analysis.leak_calculator import gen_classification_metrics, print_classification_metrics, save_classification_metrics

class CircuitBasedNaiveExtractionAttack(ExtractionAttack):
    ...
    def _get_tagger(self):
        if self._tagger is None:
            ...
            ner_args = NERArgs(
                ner="flair", ner_model="flair/ner-english-ontonotes-large"
            )
    ...
    def attack(self, lm: LanguageModel, *args, **kwargs) -> dict:
        ...
        generated_text: GeneratedTextList = lm.generate(sampling_args=sampling_args, hooked_lm=self.hooked_model)

        pii_mentions = []
        for batch_text in tqdm(
            batchfy(generated_text, self.env_args.eval_batch_size), desc="attack"
        ):
