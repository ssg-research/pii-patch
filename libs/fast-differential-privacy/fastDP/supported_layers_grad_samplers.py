...
def _light_linear_weight_norm_sample_sequential(A, B):
    ...
    Multi-GPU compatible version with simplified device selection.
    """
    # Simple device alignment for JIT compatibility
    if A.device != B.device:
        # Always move B to A's device to avoid complex device logic in JIT
        B = B.to(A.device)
